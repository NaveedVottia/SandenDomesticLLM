...
const MODELS_TO_TEST = ['claude_sonnet']; // Only test working model
const TEST_RESULTS = [];
...
class ModelTester {
  ...
  async sendMessage(message, role = 'user') {
    try {
      ...
      // Parse streaming format to extract the actual response
      let fullResponse = '';
      ...
      // Log conversation
      this.results.conversation.push({
...
async function runAllTests() {
  console.log('🚀 Starting SDK5 v2 Model Comparison Tests');
  ...
  for (const modelName of MODELS_TO_TEST) {
    ...
    const tester = new ModelTester(modelName);
  ...
  for (const result of TEST_RESULTS) {
    ...
    console.log(`💬 Conversation Length: ${result.conversation.length} messages`);
  ...
  const filename = `sdk5-v2-test-results-${timestamp}.json`;
  
  fs.writeFileSync(filename, JSON.stringify(TEST_RESULTS, null, 2));
  console.log(`\n💾 Detailed results saved to: ${filename}`);
  
  // Save conversation logs separately
  const conversationFilename = `sdk5-v2-conversation-logs-${timestamp}.json`;
  const conversationLogs = TEST_RESULTS.map(result => ({
    model: result.model,
  ...
  fs.writeFileSync(conversationFilename, JSON.stringify(conversationLogs, null, 2));
  console.log(`💬 Conversation logs saved to: ${conversationFilename}`);
...
export { ModelTester, runAllTests };
