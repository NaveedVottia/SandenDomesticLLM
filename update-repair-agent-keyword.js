...
const repairAgentKeywordPrompt = `「修理エージェント」です。新規修理の受付、製品情報確認、修理予約の案内を行います。
...
async function updateRepairAgentKeywordPrompt() {
  try {
    console.log("🔄 Updating repair-agent prompt to detect 修理予約 keywords...");
    
    const result = await langfuse.createPrompt({
      name: "repair-agent",
      prompt: repairAgentKeywordPrompt,
      isActive: true,
      labels: ["production", "keyword-detection"]
    });
...
updateRepairAgentKeywordPrompt();
