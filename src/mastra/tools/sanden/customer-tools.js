...
function getArgs(args) {
    ...
    if (args && typeof args === "object") {
        for (const [key, value] of Object.entries(args)) {
            if (key === "writer" || key === "mastra" || key === "input" || key === "context")
                continue;
...
function mapLookupKey(worksheet, key) {
...
function extractRowsFromZapier(result) {
    if (!result)
        return [];
    // Direct rows array
...
async function mcpLookupRows(worksheet, lookup_key, lookup_value) {
    try {
...
async function mcpListRows(worksheet, row_count = 500) {
    try {
...
async function validateEmailOnline(email) {
    try {
...
async function validatePhoneOnline(phone) {
    try {
...
async function validateCompanyNameOnline(companyName) {
    try {
...
export const identifyCustomerWithFullDetails = createTool({
    ...
    async execute(args) {
        const { companyName, email, phone, allDetails } = getArgs(args);
            ...
            for (const strategy of searchStrategies) {
                try {
                    ...
                    if (result && Array.isArray(result) && result.length > 0) {
                        ...
                        const bestMatch = result.find((row) => {
...
export const customerTools = {
                ...
                if (/[a-zA-Z\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF]/.test(query)) {
                    ...
                    if (!merged.length) {
                        ...
                        if (qNoSpace && qNoSpace !== query) {
                            ...
                            if (byCompanyNoSpace?.length)
                                merged.push(...byCompanyNoSpace);
                        }
                    }
                }
    ...
    getCustomerByDetails: createTool({
        ...
        execute: async ({ context, writer }) => {
            ...
            if (!effectiveCustomerId && missing.length) {
                const order = ["companyName", "email", "phone"];
                const askNext = order.find(f => missing.includes(f));
                if (writer && askNext) {
                    const jpMap = { companyName: "会社名", email: "メールアドレス", phone: "電話番号" };
                    try {
                        writer.write(`お客様情報の確認のため、次の項目を教えてください：${jpMap[askNext]}`);
                    }
                    catch { }
                }
                return {
                ...
                if (effectiveCustomerId && idPreferred) {
                    ...
                    if (!results.length && (normalizedCompanyName || email || phone)) {
                        ...
                        const fuzzy = rows.filter((r) => {
                            ...
                            const tests = [];
                            if (normalizedCompanyName)
                                tests.push(name.includes(normalizedCompanyName));
                            if (email)
                                tests.push(em.includes(String(email).toLowerCase()));
                            if (phone)
                                tests.push(ph.includes(String(phone)) || ph.replace(/\D/g, "").includes(String(phone).replace(/\D/g, "")));
    ...
    logAccess: createTool({
        ...
        execute: async ({ context }) => {
            ...
            try {
                ...
                const result = { success: true, logId: `log_${Date.now()}` };
                if (result.success) {
                    return {
                        success: true,
    ...
    escalateToHuman: createTool({
        ...
        execute: async ({ context }) => {
            const { reason, priority, sessionId, customerId, context: contextData, } = context;
            try {
                try {
                    await zapierMcp.callTool("google_sheets_create_spreadsheet_row", {
                ...
                const result = { success: true, escalationId: `esc_${Date.now()}`, estimatedResponseTime: priority === "Emergency" ? "即座" : "2時間以内" };
                if (result.success) {
                    return {
                        success: true,
