...
let mastraInstance;
export function setMastraInstance(instance) {
...
function extractDataFromResponse(response, dataType) {
...
export const delegateTo = createTool({
    ...
    async execute(args) {
        const { writer, mastra } = args;
        const parsed = getArgs(args);
        ...
        try {
            const instance = mastra || mastraInstance;
            ...
            if (agentId === "customer-identification")
                extractedData = extractDataFromResponse(fullResponse, "CUSTOMER");
            else if (agentId === "repair-agent")
                extractedData = extractDataFromResponse(fullResponse, "PRODUCT");
            else if (agentId === "repair-history-ticket")
                extractedData = extractDataFromResponse(fullResponse, "ISSUE");
            else if (agentId === "repair-scheduling")
                extractedData = extractDataFromResponse(fullResponse, "REPAIR");
            ...
            return { ok: false, agentId, extractedData: null };
        }
    },
});
...
export const escalateToHuman = createTool({
    ...
    async execute(args) {
        const { reason, priority, context } = getArgs(args);
...
export const validateContext = createTool({
    ...
    async execute(args) {
        const { context, requiredFields, schema } = getArgs(args);
        const traceId = await langfuse.startTrace("tool.validateContext");
        if (schema)
            return { isValid: true, missingFields: [], message: "ok" };
        const missing = [];
...
export const lookupCustomerFromDatabase = createTool({
    ...
    async execute(args) {
        const { storeName, searchType = "store_name" } = getArgs(args);
...
export const logCustomerData = createTool({
    ...
    async execute(args) {
        const { customerData, source } = getArgs(args);
