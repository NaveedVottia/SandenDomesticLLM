...
async function loadLangfusePrompt() {
  try {
    ...
    const promptClient = await langfuse.getPrompt("Domestic-customer-identification", undefined, { cacheTtlSeconds: 1 });
    if (promptClient?.prompt?.trim()) {
      CUSTOMER_IDENTIFICATION_INSTRUCTIONS = promptClient.prompt.trim();
      console.log(`[Langfuse] ✅ Loaded Domestic-customer-identification prompt via SDK (v${promptClient.version})`);
    }
  } catch (error) {
  ...
  execute: async (args: any) => {
    ...
    if (result.found && result.customerData && result.customerData.customerId) {
      try {
        ...
        sharedMemory.set("currentAgent", "Domestic-customer-identification");
