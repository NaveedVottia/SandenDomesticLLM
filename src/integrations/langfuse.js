...
export class LangfuseIntegration {
    constructor() {
        this.langfuse = null;
        this.langfuseTracing = null;
        this.enabled = false;
        this.tryInitFromEnv();
    }
    tryInitFromEnv() {
        if (this.enabled && this.langfuse && this.langfuseTracing)
            return;
    ...
    async getPromptText(promptName, label = "production") {
        try {
            this.tryInitFromEnv();
            ...
            console.log(`[Langfuse] Response received using method '${methodUsed}':`, typeof response, response ? "has data" : "no data");
            ...
            if (response) {
                ...
                console.log(`[Langfuse] Full response:`, JSON.stringify(response, null, 2));
            }
            ...
            if (typeof response === "object") {
                ...
                for (const prop of possibleTextProperties) {
                    if (response[prop] && typeof response[prop] === "string") {
                        console.log(`[Langfuse] Successfully extracted prompt text from property '${prop}': ${promptName}`);
            ...
            throw new Error(`Unable to extract prompt text from response for name="${promptName}"`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : "Error";
            console.error(`[Langfuse] Error fetching prompt "${promptName}":`, errorMessage);
            // Fallback to empty string to allow continuation
            return "";
        }
    }
    async logPrompt(promptName, input, output, metadata) {
        try {
            this.tryInitFromEnv();
            ...
            console.log(`[Langfuse Tracing] Successfully logged prompt: ${promptName}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : "Error";
    ...
    async logToolExecution(traceId, toolName, input, output, metadata) {
        try {
            this.tryInitFromEnv();
    ...
    async endTrace(traceId, finalMetadata) {
        try {
            this.tryInitFromEnv();
            if (!this.enabled || !this.langfuseTracing || (traceId && traceId.startsWith('trace_'))) {
                // This is a fallback trace ID, just log locally
