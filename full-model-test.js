...
const MODELS_TO_TEST = ['karakuri', 'tsuzumi', 'plamo', 'nova_micro', 'nova_pro', 'claude_sonnet'];
const TEST_RESULTS = [];
...
async function runAllTests() {
  console.log('🚀 Starting Full Model Comparison Tests');
  ...
  const filename = `full-model-test-results-${timestamp}.json`;
  
  fs.writeFileSync(filename, JSON.stringify(TEST_RESULTS, null, 2));
  console.log(`\n💾 Detailed results saved to: ${filename}`);
  
  // Save conversation logs separately
  const conversationFilename = `conversation-logs-${timestamp}.json`;
