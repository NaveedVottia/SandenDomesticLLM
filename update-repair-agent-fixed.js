...
const repairAgentFixedPrompt = `「修理エージェント」です。新規修理の受付、製品情報確認、修理予約の案内を行います。
...
async function updateRepairAgentFixedPrompt() {
  try {
    console.log("🔄 Updating repair-agent prompt to fix delegation...");
    
    const result = await langfuse.createPrompt({
      name: "repair-agent",
      prompt: repairAgentFixedPrompt,
      isActive: true,
      labels: ["production", "fixed-delegation"]
    });
